...
enum lval{
    a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z,zero,one,two,three,four,five,six,seven,eight,nine,period,exclamation,question,comma
};

Banner::Banner()
    :   Banner_(BANNER_LAYER),
        Text_(TEXT_LAYER)
{
    BannerTexture_ = new Texture(banner_path_, BANNER_LAYER);
    TextTexture_ = new Texture(font_path_, TEXT_LAYER);

    Banner_.Objects_.push_back(new GameObject("res/models/Banner.obj",BANNER_LAYER, {0.0f,0.0f,0.0f}));

    std::string msg = "lets.fucking.go!";
    WriteText(msg);
...
void Banner::WriteText(std::string& msg)
{
    ...
    for(int i = 0; i < (int)num_letters_; i ++)
    {
    ...
    for(int i = 0; i < (int)positions.size(); i++)
        Text_.Objects_.push_back(new GameObject("res/models/Letter.obj", TEXT_LAYER, positions.at(i), GetTextureCoords(msg.at(i))));
}

void Banner::UpdateBufferData(std::vector<float>& vertices, std::vector<unsigned int>& inidces, int& inidicePos)
{
